...
## 2026-09-08 · P3-S4 사전 결정 — 위젯 전환 시각 KST 07:00, prefetch 불필요 확정

**Task**: [P3-S4](../phase-3-widget.md#s4-갱신딥링크안정화-srs-42) 사전 설계 (P3-S2-T4 착수 전 선결)
**한 일**: 직전 로그에서 "아직 결정 안 했다"고 남겨둔 **자정 갱신 공백 / prefetch 허용 여부**를 확정했다. 결론은 **prefetch 안 함 — 백엔드가 뷰·RLS를 열 필요 없음**이다.
- 사람이 **위젯 전환 시각을 KST 오전 7시**로 정했다. 곡은 KST 자정 cron으로 발행되므로 7시에 보여줄 곡은 **7시간 전부터 이미 `published`**다. 미발행 콘텐츠를 미리 받을 상황 자체가 생기지 않는다.
- `@ongod/core`에 `WIDGET_SWITCH_HOUR_KST = 7` 상수 추가(앱·iOS·Android 공유).
- `docs/phase-3-widget.md` S4에 동작 명세 기재, `handoff.md`의 관련 항목 두 곳을 결론으로 갱신.
- `syncWidget.ts`의 이제 틀린 주석 수정 — "어제 곡이 남으면 사용자가 오늘 곡인 줄 안다"고 써뒀는데, 7시 전환에서는 **자정~7시에 어제 곡이 떠 있는 게 정상 동작**이다.
**왜 이렇게**:
- **내 원래 우려는 전제가 틀렸다.** "자정에 즉시 전환"을 가정했기 때문에 자정 이전에 미리 받아야 했고, 그 시점엔 `scheduled`라 RLS를 열어야 한다는 결론이 나왔다. 전환 시각이 7시가 되면서 그 전제가 사라졌다 — 데이터를 받는 시점에 이미 공개돼 있다.
- **부수 효과가 크다.** iOS 백그라운드 fetch는 실행 시점이 OS 재량이라 "자정 직후 즉시"를 보장할 수 없는 게 원래 문제였는데, **자정~7시의 7시간 창** 안에서 한 번만 돌면 되므로 사실상 확실해진다. 게다가 타임라인에 "7시에 이걸로 바꿔라"를 예약해두면 백그라운드 fetch가 안 돌아도 정확한 시각에 전환된다 — 원래 iOS 위젯 타임라인의 용도다.
- **KST 7시로 정한 이유**: 사용자 로컬 7시가 아니다. 곡 날짜(`pick_date`) 자체가 KST 기준으로 정의돼 있어서, 로컬 기준을 섞으면 "어느 날의 곡인가"와 "언제 바뀌는가"가 서로 다른 타임존을 보게 된다. 상수를 `@ongod/core`에 둔 것도 같은 이유 — 앱·iOS·Android 세 곳이 각자 박으면 반드시 어긋난다.
- **엣지 케이스 처리**: 자정~7시 사이 위젯 첫 설치 시 보여줄 "어제 곡"이 없다. 빈 위젯보다 오늘 곡을 바로 띄우는 쪽을 택했다(사람 승인). 잠깐 이르게 바뀌는 것이 빈 화면보다 낫다.
**변경 파일**: `packages/core/src/domain/widget.ts`, `apps/mobile/lib/widget/syncWidget.ts`(주석), `docs/phase-3-widget.md`, `docs/logs/handoff.md`
**검증**: `pnpm turbo run typecheck lint test` 19/19. (동작 검증은 P3-S2-T4 구현 후 EAS 빌드에서)
**막힌 점 / 다음 할 일**:
...
## 2026-09-09 · P3-S2-T4 — 위젯 타임라인: KST 07:00 전환 예약

**Task**: [P3-S2-T4](../phase-3-widget.md#s2-ios-위젯-widgetkit)
**한 일**: 위젯이 **아침 7시에** 새 곡으로 바뀌도록 타임라인을 예약한다. 그전까지는 `updateSnapshot`으로 "지금 당장" 그리기만 했다.
- `@ongod/core`에 시각 판단을 추가했다: `widgetSwitchAt(now)`(그날 KST 07:00), `isAfterWidgetSwitch(now)`, 그리고 **순수 함수 `buildWidgetTimeline`**.
- `lib/widget/syncWidget.ts`를 `updateTimeline`으로 전환. 07:00을 지났으면 오늘 곡을 지금, 아직 전이면 지금 떠 있는 것(어제 곡)을 유지하고 07:00에 전환을 예약한다.
- **커버 정리 로직을 바꿨다** — 아래 "왜 이렇게" 참고.
**왜 이렇게**:
- **시각 판단을 순수 함수로 `@ongod/core`에 뺐다.** 이유가 둘이다. (1) Android(P3-S3)는 WorkManager로 같은 규칙의 예약을 잡아야 하는데, 두 플랫폼이 다르게 판단하면 같은 날 다른 곡이 뜬다. (2) 현재 KST 15시라 "07:00 이전" 경로를 실기기에서 자연스럽게 탈 수가 없다 — 순수 함수로 빼두면 두 경로를 테스트로 완전히 덮을 수 있다(설계 원칙 5).
- **"어제 곡 유지" 엔트리를 명시적으로 다시 넣는다.** 미래 엔트리 하나만 던지면 07:00까지 위젯이 무엇을 그릴지가 불확실하다. `getTimeline()`으로 지금 그려지는 항목을 읽어 첫 엔트리로 되돌려 넣어, "지금은 이것 / 07:00부터 저것"을 명시한다.
- **커버 파일 정리 기준을 "오늘 것만 남김" → "타임라인이 참조하는 것만 남김"으로 바꿨다.** 이게 없었으면 07:00 이전 경로에서 버그가 났다: 어제 곡 엔트리를 유지하는데 그 이미지 파일을 지워버리면 위젯이 07:00까지 **커버 없이** 보인다. 그래서 다운로드(`cacheCover`)와 정리(`pruneCovers`)를 분리하고, 정리는 타임라인을 다 만든 뒤 참조되지 않는 파일만 지운다.
- 첫 설치(유지할 어제 곡이 없음) 시 07:00 전이라도 오늘 곡을 바로 띄운다 — 사람 결정(2026-09-09).
**변경 파일**: `packages/core/src/domain/widget.ts`, `packages/core/src/domain/widget.test.ts`, `apps/mobile/lib/widget/syncWidget.ts`, `docs/phase-3-widget.md`
**검증**:
- `@ongod/core` 테스트 **26개** 통과(위젯 19 + KST 7). 신규: 전환 시각 경계(06:59:59 / 07:00:00 / 자정 직후), UTC 날짜와 KST 날짜가 다른 순간, `buildWidgetTimeline`의 세 분기(07:00 이후 / 이전+유지 / 이전+첫 설치) + 픽 없는 날.
- `pnpm turbo run typecheck lint test build` 20/20.
- **실기기(시뮬레이터 dev 빌드) 확인**:
  - 07:00 이후 경로 — `[widget] 타임라인 [{at: 2026-09-09T06:30:34Z, title: "Take My Hand, Precious Lord"}]` 엔트리 1개.
  - **cron으로 곡이 실제 교체된 것을 위젯에서 확인** — 어제 "Oh Happy Day"였던 위젯이 오늘 "Take My Hand, Precious Lord"(Mahalia Jackson)로 커버까지 바뀌어 그려졌다. 파이프라인→뷰→위젯 전 구간이 실제 운영 경로로 검증된 것.
  - 07:00 이전 경로 — 시각을 12시간 앞으로 임시 조정해 재현. `[{at: 2026-09-09T06:30:34Z(유지)}, {at: 2026-09-09T22:00:00Z}]` 2개 엔트리가 나왔고, 두 번째가 **정확히 2026-09-10 07:00 KST**다. `getTimeline()` 왕복(이전 엔트리를 읽어 유지)도 이때 함께 확인됐다. 확인 후 임시 코드 원복(`grep TEMP` 0건).
**막힌 점 / 다음 할 일**:
...
## 2026-09-11 · P3-S4-T2 — 타임존 정합성: 서버 날짜 기준 전환 + 다중 타임존 검증

**Task**: [P3-S4-T2](../phase-3-widget.md#s4-갱신딥링크안정화-srs-42) (+ S1 Step 체크박스 정정)
**한 일**:
- **날짜·시각 판단 전수 점검.** "오늘이 며칠인가" 또는 "언제 바꾸는가"를 판단하는 곳을 저장소 전체에서 찾아 대조했다.

  | 위치 | 판단하는 것 | 기준 |
  |------|-------------|------|
  | pg_cron `publish-daily-picks-kst-midnight` | 발행 시각 | `0 15 * * *` UTC = KST 00:00 (backend가 `cron.job_run_details`로 매일 정시 실행을 확인해둠) |
  | `publish_scheduled_daily_picks()` | 발행할 날짜 | `(now() at time zone 'Asia/Seoul')::date` |
  | `widget_today_pick` 뷰 | 위젯의 "오늘" | 위와 **완전히 같은 식** |
  | `@ongod/core` `toKstDateString`·`kstMidnightToUtc` | 앱·어드민의 "오늘" | UTC+9 고정 — KST는 1988년 이후 서머타임이 없어 Postgres `Asia/Seoul`과 일치 |
  | 앱 `useTodayPick`·`useRecentPicks`·`app/index.tsx` | 오늘 카드 | `toKstDateString()` |
  | 어드민 대시보드·예약 화면 | 달력·빈 날짜 | `toKstDateString` + `Date.UTC`/`getUTC*`, 날짜 입력은 `YYYY-MM-DD` 문자열 그대로 저장 |
  | `@ongod/core` `buildWidgetTimeline` | 위젯 전환 시각 | KST 07:00 |

  기기 로컬 시간 API(`getHours`·`setHours`·`getDate()`·`toLocaleDateString`·`Intl.DateTimeFormat` 등)는 앱·어드민·공유 패키지 어디에도 **0건**이다.
- **보완 — 위젯 전환 시각을 서버가 준 `pickDate`로 계산한다.** `widgetSwitchAtForDate(kstDate)` 추가, `buildWidgetTimeline`에 `pickDate` 옵션 추가, `syncWidget`이 페이로드의 `pickDate`를 넘긴다. 오늘 픽이 없어 날짜를 모를 때만 기존처럼 기기 시계로 판단한다.
- **다중 타임존 회귀 테스트** `packages/core/src/timezone.test.ts` 신규 — 9개 타임존에서 KST 날짜·KST 자정 해석·07:00 전환·경계 판정·타임라인 예약 시각이 모두 같은 UTC 순간을 내는지 확인.
- **문서 정합성**: `phase-3-widget.md`에 남아 있던 "자정 갱신" 문구를 07:00 결정에 맞춰 고쳤다(DoD·종료 기준·S3-T4·S4-T4). 특히 S3-T4는 "WorkManager 자정"이라 그대로 두면 Android를 자정 기준으로 구현할 위험이 있었다. "방식" 줄도 ADR-0004 단독 → iOS는 ADR-0009로 갱신. `widget.ts` 머리 주석의 "이 파일에 날짜 계산이 없는 것은 의도된 것이다"는 P3-S2-T4 이후 틀린 문장이라 역할 분담 설명으로 바꿨다.
- **S1 Step 체크박스 정정** — T1~T4가 모두 끝났는데 Step 줄이 `[ ]`·"T2만 남음"으로 남아 있었다. P3-S1-T2 완료 때 내가 빠뜨린 것.
- `.claude/launch.json`에 `mobile`(Metro) 구성 추가 — 개발 빌드 검증 때 Metro를 매번 같은 방식으로 띄우기 위해.

**왜 이렇게**:
- **점검에서 나온 불일치는 "어느 시계를 보느냐" 한 곳이었다.** 어느 곡인지는 **서버 시계**(뷰의 `now()`)가, 언제 바꿀지는 **기기 시계**가 정하고 있었다. 시계가 몇 분 늦은 폰이 KST 9일 00:02에 동기화하면 서버는 9일 곡을 주는데 기기는 아직 8일 23:57이라 "8일 07:00은 지났다"로 판단해 **9일 곡을 07:00까지 기다리지 않고 자정에 바로 띄운다.** 곡 자신의 날짜로 전환 시각을 구하면 두 판단이 같은 출처를 보게 된다. 수동으로 시계를 맞춘 폰은 드물어 영향은 작지만, 수정이 인자 하나라 싸고, **P3-S3 Android가 이 함수를 그대로 쓰므로** 구현 전에 고치는 게 맞다고 봤다.
- **다중 타임존 테스트를 넣은 이유 — 기존 검증에 사각지대가 있었고, 실제로 확인했다.** 이 로직은 지금까지 KST 기기(개발 Mac·시뮬레이터)와 UTC(CI)에서만 돌았다. KST 기기에서는 "`+09:00`을 무시하고 로컬 시각으로 해석하는" 버그가 우연히 같은 답을 낸다. **변이 테스트**로 `kstMidnightToUtc`에서 `+09:00`을 지운 채 돌려보니:
  - 기존 테스트, 이 Mac: **32/32 전부 통과 — 버그를 못 잡는다.**
  - 기존 테스트, `TZ=UTC`(CI 조건): 11개 실패로 잡힌다. 다만 UTC와 우연히 맞아떨어지는 종류의 버그는 CI도 못 잡는다.
  - 새 타임존 테스트: KST를 뺀 8개 타임존 전부에서 잡힌다(32개 실패).
  - 원복 후 86/86 통과.
- **테스트가 헛돌지 않게** 각 타임존의 첫 케이스가 `getTimezoneOffset()`이 기대값인지부터 확인한다. 런타임이 `TZ` 변경을 무시하면 나머지가 전부 의미 없이 통과할 수 있기 때문이다.
- 타임존 선택 기준: 서머타임(LA·뉴욕·런던), 30·45분 단위 오프셋(로드하우·카트만두), 기기 날짜가 KST보다 앞서는 UTC+14(키리티마티), KST와 20시간 차인 UTC-11(파고파고). 해외 서머타임 전환일(3/8·10/25·11/1)의 전환 시각도 확인한다.
- **Hermes(앱의 실제 JS 엔진)에서 따로 확인했다.** Node(V8) 테스트만으로는 Hermes의 날짜 문자열 해석이 같다는 보장이 없고, 지금까지 시뮬레이터 검증은 전부 KST 타임존에서 해서 이 차이를 드러낼 수 없었다.

**변경 파일**: `packages/core/src/domain/widget.ts`, `packages/core/src/domain/widget.test.ts`, `packages/core/src/timezone.test.ts`(신규), `apps/mobile/lib/widget/syncWidget.ts`, `.claude/launch.json`, `docs/phase-3-widget.md`
**검증**:
- `@ongod/core` 테스트 **86개** 통과(기존 26 + pickDate 기준 전환 6 + 타임존 54). `pnpm turbo run typecheck lint test build` **20/20**.
- 변이 테스트: 위 "왜 이렇게" 참고. 사각지대가 실제로 있다는 것과 새 테스트가 그걸 잡는다는 것을 둘 다 확인하고 원복했다(`kst.ts` diff 없음).
- **시뮬레이터 개발 빌드를 앱 프로세스 타임존만 바꿔 실행**(`SIMCTL_CHILD_TZ`), 임시 로그로 값 확인:
  - `America/New_York`: 오프셋 240, KST 날짜 `2026-09-09`, KST 자정 `2026-09-08T15:00:00.000Z`, 전환 시각 `2026-09-08T22:00:00.000Z`, 경계 `false`/`true` — Node 기대값과 **완전히 일치**. 실제 동기화도 이 타임존에서 `[widget] 타임라인 [{title: "Break Every Chain"}]`(오늘 dev 픽)으로 정상.
  - `Pacific/Kiritimati`(UTC+14, **기기 로컬 날짜는 이미 9/12**): 위와 같은 값, 현재 KST 날짜는 `2026-09-11` 그대로. 앱 오늘 카드도 9/11 곡. 위젯 동기화는 뉴욕에서 쓴 타임라인을 `이미 최신 — 갱신 건너뜀`으로 인식했다 — 타임라인 왕복 비교도 타임존과 무관하다.
  - 확인 후 임시 코드 원복(`grep TEMP` 0건, `_layout.tsx` diff 없음).

**막힌 점 / 다음 할 일**:
- **남겨둔 경계 두 가지** — 고치는 비용이 효과보다 크다고 판단해 수용했다.
  1. **발행 직전의 짧은 공백.** 뷰의 "오늘"은 KST 00:00:00에 바뀌지만, 픽이 `published`가 되는 건 cron이 실행을 마친 뒤다. 그 사이에 동기화가 걸리면 0행을 받아 07:00에 "준비 중"을 예약한다. 다음 동기화(백그라운드 4시간 주기 또는 앱 열기)에서 바로잡힌다. cron 실행 소요 시간 자체는 측정하지 않았다(짧을 것으로 보지만 수치 확인은 안 함).
  2. **틀린 기기 시계.** 예약 발동은 OS(WidgetKit/WorkManager)가 기기 시계로 한다. 시계가 2시간 늦은 폰은 실제 09:00에 바뀐다 — 앱이 막을 수 없는 영역이다. 반면 **여행으로 기기 타임존이 바뀌는 것은 영향이 없다**(예약이 UTC 순간이라).
