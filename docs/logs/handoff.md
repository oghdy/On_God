...
**상태**: [x] 처리완료 — 프론트 세션, 2026-09-08. 계약대로 구현했다(뷰 조회·필드·폴백 3단계·딥링크 상수 전부). **실측으로 계약 검증**: `widget.webp` 512×512 36KB / `cover.webp` 600×600 46KB / 둘 다 `Cache-Control` 1년 — 문서와 일치. 날짜는 클라이언트에서 계산하지 않고 뷰에 맡겼다. **4번(자정 갱신 공백)은 결정났다 — prefetch 불필요, 뷰·RLS를 열 이유가 없다.** 사람이 위젯 전환 시각을 **KST 오전 7시**로 정했기 때문이다. 곡은 KST 자정 cron으로 이미 발행되므로, 7시에 보여줄 곡은 **7시간 전부터 이미 published인 공개 데이터**다. 미발행 콘텐츠를 미리 받을 이유 자체가 사라졌다. 자세한 내용은 아래 새 항목 3번. 다만 **`packages/db`의 DB 타입에 뷰가 빠져 있어** `.from("widget_today_pick")`이 타입 에러였다 — 아래 새 항목 참고.
...
### 3. 결정됨 — 자정 갱신 공백 (백엔드 handoff 4번): **prefetch 안 한다. 뷰·RLS 그대로 두면 된다**

**결론부터: 백엔드가 할 일이 없다.** 미발행(`scheduled`) 콘텐츠를 클라이언트에 여는 요청은 하지 않는다.

**이유 — 위젯 전환 시각이 KST 자정이 아니라 오전 7시다.** 사람이 그렇게 정했다(2026-09-08).
...
7시에 보여줄 곡은 **자정부터 이미 published**다. 즉 위젯이 새 곡을 받는 시점에 그 데이터는
이미 공개돼 있고, `widget_today_pick` 뷰로 정상 조회된다. **미발행 콘텐츠를 미리 받아야 할
상황 자체가 생기지 않는다.**

원래 이 문제가 걸렸던 건 "자정에 즉시 전환"을 가정했기 때문이다. 그러면 자정 이전에 미리
받아둬야 했고, 그 시점엔 아직 `scheduled`라 RLS를 열어야 했다. 전환 시각이 7시로 정해지면서
그 전제가 없어졌다.

**부수 효과로 iOS 백그라운드 fetch의 불확실성도 해소된다.** 실행 시점이 OS 재량이라 "자정
직후 즉시"는 보장이 안 됐는데, **자정~7시 사이 7시간 창** 안에서만 한 번 돌면 되므로 사실상
확실하다. 앱을 그 사이에 한 번이라도 열면 그때 갱신되고, iOS 타임라인에 "7시에 이걸로
바꿔라"를 예약해두면 백그라운드 fetch 없이도 정확한 시각에 전환된다 — 이게 원래 위젯
타임라인의 용도다.

**자정~7시 사이에 어제 곡이 떠 있는 것은 버그가 아니라 의도된 동작이다.** 이 창을 없애려고
`published` 이전 데이터를 여는 방향은 앞으로도 택하지 않는다.

부수 결정 둘(프론트 소관, 알아만 두면 됨):
- **전환 시각 기준은 KST 07:00**이다(사용자 로컬 07:00이 아님). 곡 날짜 자체가 KST 기준이라
  그쪽에 맞췄다. 앱·iOS·Android가 같은 값을 봐야 해서 `@ongod/core`에 상수로 뒀다.
- 사용자가 **자정~7시 사이에 위젯을 처음 설치**하면 보여줄 "어제 곡"이 없다. 이때는 빈 위젯
  대신 오늘 곡을 바로 띄운다.
