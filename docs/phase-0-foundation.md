...
- [x] 🤖 **P0-S6-T6d** — admin typecheck `.next/types` 레이스 제거 — typecheck 전용 `tsconfig.typecheck.json`으로 `.next` 완전 분리([로그](./logs/backend-log.md#2026-09-08--p0-s6-t6d--admin-typecheck-nexttypes-레이스-제거))
  *`exclude`만으로는 `next-env.d.ts`의 `reference path`가 우회해 안 끊긴다 — 동시 실행 3회 연속 20/20으로 검증*
