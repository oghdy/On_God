...
- [x] **P3-S3-T1b** 🤝 Android EAS 빌드용 앱 서명 키(keystore) 생성 승인 — **완료 (2026-09-12 승인 → 에이전트가 빌드 실행)**
  *무엇을: "EAS가 Android 서명 키를 만들어 보관해도 된다"는 승인 한 마디. 계정 가입·비밀번호 입력은 필요 없다(EAS 로그인은 이미 돼 있음).*
  *왜: 이 앱은 EAS에서 Android를 한 번도 빌드한 적이 없어, 첫 빌드 때 서명 키가 새로 만들어진다. 이 키는 나중에 Play Store에 올라갈 앱의 "신원"이라 한번 쓰기 시작하면 바꾸기 번거롭다 — 그래서 에이전트가 혼자 만들지 않았다.*
  *결과: EAS가 서명 키를 만들어 자기 서버에 보관한다(`Created keystore`). 빌드 성공, APK를 에뮬레이터에 설치해 위젯까지 정상 확인. 이 키는 앞으로 이 앱의 Android 빌드에 계속 쓰인다 — Play Store 업로드 키가 되므로 EAS 계정을 잃지 않도록 관리가 필요하다.*
  *~~지금 막혀 있는 것: 없음. Android 위젯은 로컬 빌드(에뮬레이터)로 검증을 끝냈다. 승인되면 EAS Android 개발 빌드로 관리형 빌드 경로까지 확인하고, 그 빌드를 실기기 테스트(P3-S3-T7)에 쓴다.~~*
