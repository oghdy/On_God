...
/** 지금 화면에 그려지고 있는 타임라인 엔트리(= `date <= now` 중 가장 최근 것). */
async function currentEntry(
  widget: { getTimeline(): Promise<{ date: Date; props: OnGodTodayProps }[]> },
  now: Date,
): Promise<{ date: Date; props: OnGodTodayProps } | undefined> {
  try {
...
async function deliverWidgetPayload(payload: WidgetPayload | null): Promise<void> {
  ...
  // 커버를 지우기 전에 지금 떠 있는 것을 먼저 읽어둔다 — 그 엔트리가 참조하는 파일을
  // 살려둬야 하기 때문이다(`pruneCovers` 주석 참고).
  const current = await currentEntry(OnGodTodayWidget, now);
