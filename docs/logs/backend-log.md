...
## 2026-09-07 · P1-S4-T5 후속2 — 앨범커버 백필 (기존 파일 캐시 헤더 갱신)

**Task**: [P1-S4-T5](../phase-1-content-pipeline.md#s4-곡-등록--오케스트레이션) (사람 승인 후 진행)
**한 일**: dev `album-covers` 버킷의 기존 파일(`89f3b79c-.../cover.webp`, `thumbnail.webp` — 유일한 곡)을 다운로드 후 동일 바이트로 재업로드하며 `cacheControl: max-age=31536000` 지정. prod 버킷은 조회 결과 비어있어(발행된 곡 없음) 대상 없음.
**왜 이렇게**: 사람에게 "필수는 아니고 개선"이라고 정확히 안내한 뒤 승인받고 진행. 사람이 승인하기 전에 "재업로드 = 원본 재다운로드 → sharp 재처리"로 오해할 수 있어서, **원본 재처리 없이 이미 Storage에 있는 WebP 바이트를 그대로 내려받아 그대로 다시 올리는 것**임을 재업로드 전에 스스로 확인해뒀다 — Storage API의 업로드는 콘텐츠와 무관하게 메타데이터(`cacheControl`)만 갱신하는 별도 엔드포인트가 없어서, 바이트를 다시 보내는 것 외엔 방법이 없었음
**변경 파일**: 없음(코드 변경 아님 — Storage 오브젝트만 재업로드)
**검증**:
- 재업로드 응답의 `Id`가 기존 오브젝트 ID와 동일(같은 오브젝트 in-place 갱신, 새 오브젝트 생성 아님) 확인
...
## 2026-09-08 · P1-S6-T3 후속 + 시크릿 정책 정정 — cron 실제 동작 확인, DB 비밀번호 소재 정정

**Task**: [P1-S6-T3](../phase-1-content-pipeline.md#s6-예약-발행-시스템), [P0-S5](../phase-0-foundation.md#s5-환경시크릿-관리)
**한 일**: 두 가지 미결·오기재 사항을 실측으로 닫았다.

**1. pg_cron이 실제로 매일 도는지 확인 (P1-S6 로그의 미결 항목)**
2026-08-29 로그에 "실제 KST 자정에 cron이 저절로 도는지는 시간이 지나야 확인된다"고 남겨뒀던 것을, 10일치 이력이 쌓인 지금 `cron.job_run_details`로 직접 확인했다.

- **dev: 08-29 ~ 09-07 매일 `15:00:00` UTC에 실행, 10건 전부 `succeeded`** (UTC 15:00 = KST 00:00 정확히 일치)
- **prod: 09-03 ~ 09-07 확인분 전부 `succeeded`**
- 모든 실행의 `return_message`가 `0 rows` — **할 일이 없어서 0건이었을 뿐 cron 자체는 한 번도 거르지 않았다**

이건 이번 세션의 콘텐츠 진단("큐가 비어서 아무것도 발행 안 된 것이지 cron 문제가 아니다")을 추측이 아니라 **실행 이력으로 확정**해준다. 동시에 **P3-S4-T2(발행 cron ↔ 위젯 갱신 타임존 정합성)의 백엔드 쪽 전제도 검증된 셈**이다 — 위젯이 기준으로 삼을 "KST 자정에 새 픽이 published로 바뀐다"가 실제로 그 시각에 일어난다는 게 10일치 실측으로 확인됐다.

**2. DB 비밀번호 소재 정정 (`docs/secrets-policy.md`)**
`secrets-policy.md`는 DB 비밀번호를 `/tmp/ongod_{dev,prod}_dbpw.txt`에 둔다고 적고 있었는데 **두 파일 다 존재하지 않는다.** `/tmp`는 macOS가 재부팅 등으로 비우는 경로라 secret 보관처로 부적절했다 — "`.env`에 평문으로 안 넣는다"는 결정 자체는 옳았지만 대체 보관처 선택이 틀렸다.

한편 `.env`에는 `SUPABASE_*_DB_PASSWORD` 키가 값과 함께 들어있는데 **그 값으로는 인증이 안 된다**(직전 세션에서 pooler 접속 시도 → `password authentication failed`. 리전·테넌트 인식은 정상이었으니 비밀번호만 틀린 것). 즉 문서 서술과 `.env` 실제 내용이 서로 어긋난 채 방치돼 있었다.

**왜 이렇게**: 직전 세션에서 "`.env`의 값이 유효하지 않다"까지만 적고 넘어갔는데, 이번에 `secrets-policy.md`를 읽다가 **애초에 문서가 다른 곳을 가리키고 있었다**는 걸 발견했다. 둘 중 하나만 고치면 다음 세션이 또 같은 혼란을 겪으므로 표와 주석을 같이 정정했다. **지금 당장 문제가 되지는 않는다** — DB 비밀번호가 필요한 건 psql 직접 접속과 `supabase db push`뿐이고, 지금까지 모든 스키마 작업은 Management API(임시 PAT)나 service_role 키로 처리해왔다. 필요해지면 대시보드에서 재설정하고 `.env`를 갱신하는 게 맞다(gitignore되므로 사라지는 `/tmp`보다 낫다)는 것까지 문서에 적었다.
